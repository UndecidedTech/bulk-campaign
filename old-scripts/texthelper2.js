  ...
  for (let i = 0; i < pFile.length; i++) {
    try  {
      ...
      if (providerResult.status === "success") {
        console.log("Succeeded for: ", pFile[i][1])
        success.push(providerResult);
      } else if (providerResult.status === "failed") {
        console.log("Failed for: ", pFile[i][1])
        failed.push(providerResult);
      } else {
        console.log("IS NOT A TARGET: ", pFile[i][1])
...
async function sendText(provider) {
  let providerObj = {
    ...bodyObj,
    "custom_replacement_fields": {
      "user_name": provider[1],
      "company_name": provider[0]
    },
    "recipient_list": [
      {
        "contact": {
          "phone_number": `+1${provider[2]}`,
        }
      }
    ]
  }
